...
| Flame aura (visual) | Replaces the fire-ring attempt: procedural `gem_fire.gdshader` (`render_mode unshaded`) body aura — warm mottled fbm cracks plus a hot rim — masked by the gem's own base-texture alpha (FireRing sized to Base's exact drawn footprint so UVs line up 1:1; erosion-derived rim hugs any silhouette), so the aura cannot bleed past the gem art. Centered Flame icon dropped: the aura alone is the cue; Star keeps its overlay. Fixed fire palette; per-gem tinting deferred. If the aura alone proves too subtle on-device, re-enable the icon (one line in `GemActor.ApplyAppearance`) |